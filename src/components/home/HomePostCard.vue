<template>
  ...
  >
    ...
    <div class="flex-1 flex flex-col justify-between min-w-0">
      ...
      <!-- 互动按钮区域 -->
      <div class="flex items-center text-gray-400 mt-3 pl-0 relative z-10" @click.stop>
        <!-- 点赞 -->
        <div class="flex items-center mr-6 cursor-pointer group">
          <div class="p-1.5 rounded-full group-hover:bg-blue-100 transition-all duration-200">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 group-hover:text-blue-500 transition-colors duration-200" fill="none" viewBox="0 0 24 24"
                 stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    d="M14 10h4.764a2 2 0 011.789 2.894l-3.5 7A2 2 0 0115.263 21h-4.017c-.163 0-.326-.02-.485-.06L7 20m7-10V5a2 2 0 00-2-2h-.095c-.5 0-.905.405-.905.905 0 .714-.211 1.412-.608 2.006L7 11v9m7-10h-2M7 20H5a2 2 0 01-2-2v-6a2 2 0 012-2h2.5"/>
            </svg>
          </div>
          <span class="text-xs group-hover:text-blue-500 transition-colors duration-200">{{ article.likes }}</span>
        </div>
        ...
        <div class="flex items-center mr-6 cursor-pointer group">
          ...
          <span class="text-xs group-hover:text-blue-500 transition-colors duration-200">{{ article.comments }}</span>
        </div>
        ...
        <div class="flex items-center cursor-pointer group">
          ...
          <span class="text-xs group-hover:text-blue-500 transition-colors duration-200">{{ article.favorites }}</span>
        </div>
      </div>
    </div>
